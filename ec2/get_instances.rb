...
(region,query_string) = ARGV
unless region && query_string
  puts "Usage: get_instances.rb <REGION> <QUERY_STRING>"
  exit 1
end
...
image = AWS.memoize do
  amazon_linux = ec2.instances.filter("name", query_string)
...
puts "\nTags for #{i.id}\n"
i.tags.each_pair do |tag_name,tag_value|
